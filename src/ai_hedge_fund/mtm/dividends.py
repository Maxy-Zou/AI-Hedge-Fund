...
rounded down. The credit depends only on that signal's own holding, so
...
def signal_credit(amount_cents: int, shares: int, paid: Decimal) -> int:
    """``amount_cents * shares / paid``, rounded *down* (floor).

    Floor for both signs: a dividend credit never rounds up and a withholding debit
    never rounds toward zero, so crediting a DIV and its DIVWH separately can never
    add up to more than was received (re-review).
    ...
    return int((Decimal(amount_cents) * shares / paid).to_integral_value(rounding=ROUND_FLOOR))
