...
> **Archived:** 2026-04-23 (shipped state at milestone close).
> All 36 requirements for v1.0 are marked complete. Outcomes traced via `.planning/milestones/v1.0-MILESTONE-AUDIT.md` and per-phase SUMMARY.md files under `.planning/milestones/v1.0-phases/`.
> A fresh `REQUIREMENTS.md` is created by `/gsd-new-milestone` for v1.1+.
...
- [x] **FOUND-01**: LangGraph orchestration graph defined with checkpointing and state persistence to PostgreSQL
- [x] **FOUND-02**: PydanticAI agent base with typed schemas for hypotheses, evidence, signals, and theses
- [x] **FOUND-03**: Claude model integration with dual-model routing (Haiku/Sonnet/Opus by task type)
- [x] **FOUND-04**: Langfuse observability integration for agent traces, cost tracking, and latency monitoring
- [x] **FOUND-05**: Token budget enforcement per agent and per pipeline run with configurable caps

### Data Ingestion

- [x] **DATA-01**: SEC EDGAR integration for 10-K, 10-Q, 8-K filings via edgartools with section-level chunking
- [x] **DATA-02**: XBRL financial data extraction via CompanyFacts API with pre-computed metric summaries
- [x] **DATA-03**: Equity price data ingestion (yfinance with PostgreSQL caching, Tiingo fallback)
- [x] **DATA-04**: Insider trade detection from SEC Form 4 filings with cluster buy identification
- [x] **DATA-05**: News and sentiment aggregation from Finnhub with daily per-ticker summaries
- [x] **DATA-06**: Macro context from FRED (interest rates, CPI, GDP, yield curve)
- [x] **DATA-07**: Strict temporal controls -- every data input timestamped, RAG filtered by "available as of" date
- [x] **DATA-08**: Natural language summaries generated from structured data before LLM consumption

### Single-Agent Research

- [x] **AGENT-01**: Hypothesis Generator agent can produce a structured investment thesis from SEC filing analysis
- [x] **AGENT-02**: Agent uses tool augmentation for all financial calculations (ratios, growth rates, valuation metrics)
- [x] **AGENT-03**: Thesis output includes: bull case, bear case, evidence citations, confidence score (0-100), risk factors
- [x] **AGENT-04**: Signal output includes: direction (long/short/neutral), conviction level, time horizon, position size suggestion

### Multi-Agent Specialization

- [x] **MULTI-01**: Fundamental Analyst agent -- RAG over SEC filings, financial statement analysis, valuation
- [x] **MULTI-02**: Sentiment Analyst agent -- news sentiment, earnings call tone, insider activity signals
- [x] **MULTI-03**: Technical/Quant Analyst agent -- price patterns, momentum, volatility indicators via computed tools
- [x] **MULTI-04**: Research Manager agent synthesizes analyst outputs into unified thesis with conflict resolution
...
- [x] **RISK-01**: Risk Manager agent with veto power on high-risk recommendations
- [x] **RISK-02**: Position sizing constraints enforced as hard limits (not suggestions)
- [x] **RISK-03**: Portfolio-level risk checks (sector concentration, correlation, max drawdown thresholds)
